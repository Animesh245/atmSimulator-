...
//		add(l15);
		...
		dateChooser.setBounds(300, 240, 180, 30);
...
//		
		...
		b.addActionListener(this);// to listen action
		
		
		//Setting the form background color
		...
		String state = t7.getText();
			...
			try {
				...
				Conn c1 = new Conn();	// Anything related to database use Conn class.
				String q1 = "INSERT INTO signup VALUES('"+formno+"','"+name+"','"+fName+"','"+dob+"','"+gender+"','"+email+"','"+marital+"','"+address+"','"+city+"','"+pincode+"','"+state+"')";
				c1.s.executeUpdate(q1);	// to change a query
				
				new Signup2(first).setVisible(true);	// Signup2 will be visible after creation. 
				setVisible(false);	//for current frame Signup2 will be invisible
			}
